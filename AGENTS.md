...
| Vorbis/Opus局（Listen.Moe Vorbis等）が再生できない | 実行環境のWKWebViewがOggコンテナ非対応の場合、原理的に再生不可。Radio BrowserはVorbis/Opusをまとめて`codec='OGG'`と報告するため、probeでの個別判別は原理的に不能 | `radioApi.ts`の`canPlayType()`実行時判定で、ラベルが表しうる全コーデックが再生不可と確定した局にStationCardが「非対応」バッジを表示（**一覧からの除外はしない** — 除外方式はOGGの曖昧さをどちらに倒しても誤るため。曖昧な局は表示し、再生失敗時はE4エラー表示に委ねる。OSの対応拡大に自動追随） |
