...
| ipaインストールが`0xe8008015`で弾かれる | 無料Personal Teamのプロファイルは**登録済みUDIDのリスト**を内包する。ビルド時に未接続だった実機は含まれない | 対象実機を接続した状態で`pnpm tauri ios build --export-method debugging`を回す（`-allowProvisioningUpdates`がXcodeに自動登録させる）。確認は`security cms -D -i Payload/TuneR.app/embedded.mobileprovision`の`ProvisionedDevices`。なお有効期限内のプロファイルは**再発行されず使い回される**ので、再署名しても失効日は延びない（＝失効直前に気づいたら**失効時刻を跨いでから**ビルドすると丸7日になる。2026-08-29に実証） |
...
| 実機ビルドが `Found no destinations for the scheme` (exit 70) で即失敗する | XcodeのiOSプラットフォームコンポーネント(実機デバイスサポート＋シミュレータランタイム)が消えている。**Xcodeを更新していなくても**macOSがpurgeすることがある。`iPhoneOS<ver>.sdk`は残るので`-showsdks`では気づけず、`-checkFirstLaunchStatus`もexit 0を返す(2026-08-29の再署名で遭遇) | `cd src-tauri/gen/apple && xcodebuild -showdestinations -scheme tuner_iOS -workspace tuner.xcodeproj/project.xcworkspace` の**Ineligible destinations**の理由を見る(`iOS <ver> is not installed`)。復旧は `xcodebuild -downloadPlatform iOS`(実測9分、GUI操作不要)。SDKより新しい版が入るが上位互換で解決されビルドは通る。実機しか使わなくても取得は必須 |
