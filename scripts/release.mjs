...
const PACKAGE_NAME = 'tuner'
const CRATE_NAME = 'tuner'
// Other committed version fields: [file, pattern capturing (prefix)(version), expected match count].
const EXTRA_TARGETS = [
  ['src-tauri/gen/apple/tuner_iOS/Info.plist', /(<key>CFBundle(?:ShortVersionString|Version)<\/key>\s*<string>)([^<]+)/g, 2],
  ['src-tauri/gen/apple/project.yml', /^(\s*CFBundle(?:ShortVersionString|Version): "?)([^"\s]+)/gm, 2],
]
